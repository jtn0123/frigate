...
test.describe("Classification, train filter @medium", () => {
  test.skip(({ frigateApp }) => frigateApp.isMobile, "Desktop filter popover");

  test("filtering by a class with a dash shows its train images", async ({
    frigateApp,
  }) => {
    ...
    await frigateApp.page.getByRole("button", { name: "More Filters" }).click();
