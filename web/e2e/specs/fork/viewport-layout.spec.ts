...
test.describe("Viewport layout - desktop window @high @tablet @desktop-only", () => {
  test("narrow window swaps the sidebar for the bottom bar", async ({
    ...
    await expect(base.pageRoot).toHaveCSS("left", "0px");
    await expect(base.pageRoot).toHaveCSS("bottom", "48px");
...
test.describe("Viewport layout - phone @high @mobile @mobile-only", () => {
  ...
  test("flag off keeps the bottom bar in landscape", async ({ frigateApp }) => {
    const page = frigateApp.page;
    const base = new BasePage(page, false);
    await page.addInitScript(() => {
      window.localStorage.setItem(
