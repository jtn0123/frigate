...
test.describe("Settings navigator @high", () => {
  test(
    "search finds a section by title and jumps to it",
    { tag: "@desktop-only" },
    async ({ frigateApp }) => {
  ...
  test(
    "search finds field labels in the open section and reveals the field",
    { tag: "@desktop-only" },
    async ({ frigateApp }) => {
      await installSettingsConfigRoutes(frigateApp.page);
      await frigateApp.goto(SEMANTIC_URL);
  ...
  test(
    "scrollspy rail lists the field groups of the open section",
    { tag: "@desktop-only" },
    async ({ frigateApp }) => {
      await installSettingsConfigRoutes(frigateApp.page);
  ...
  test(
    "Save All shows a before-and-after diff and saves on confirm",
    { tag: "@desktop-only" },
    async ({ frigateApp }) => {
      ...
      const { saveAll, dialog } =
        await toggleSemanticSearchAndOpenSaveAll(page);
  ...
  test(
    "compact select jumps between sections @mobile",
    { tag: "@mobile-only" },
    async ({ frigateApp }) => {
      await installSettingsConfigRoutes(frigateApp.page);
      await frigateApp.goto(SEMANTIC_URL);
