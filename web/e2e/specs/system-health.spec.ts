...
async function installDefaults(
  ...
  overrides: Parameters<FrigateApp["installDefaults"]>[0],
) {
  ...
  await frigateApp.page.route("**/api/ai/models", (route) =>
    route.fulfill({
      json: {
        ...
        audio: { status: "connected", pause_reason: "" },
        models: [],
...
test.describe("System health notices @medium @mobile", () => {
  test("System health lists notices by severity", async ({ frigateApp }) => {
    await installDefaults(frigateApp, {
      stats: QUIET_STATS,
      notices: [ERROR_NOTICE, EVENT_NOTICE],
    });
    await frigateApp.goto("/system#notices");

    await expect(
      frigateApp.page.getByLabel("Select System health"),
    ).toHaveAttribute("data-state", "on", { timeout: 15_000 });
  ...
  test("unmute deletes the row's hidden state", async ({ frigateApp }) => {
    await installDefaults(frigateApp, { stats: QUIET_STATS, notices: [] });
    await frigateApp.page.route(
      (url) =>
        url.pathname.endsWith("/api/notices") &&
        url.searchParams.get("include_hidden") === "true",
...
test.describe("System - Health notices sources @medium", () => {
  ...
  test("status bar problems stay out of the list @desktop-only", async ({
    ...
    await frigateApp.page.getByTestId("status-issues-chip").click();
  ...
  test("status bar counts shown notices next to the health text @desktop-only", async ({
    ...
    await expect(notices).toHaveAttribute("href", "/system#notices");
    await expect(
  ...
  test("status bar lists problems by severity in the compact chip @desktop-only", async ({
    ...
    const summary = frigateApp.page.getByTestId("status-issues-chip");
    await expect(summary).toBeVisible({ timeout: 15_000 });
    await expect(frigateApp.page.getByText("CPU is slow")).toHaveCount(0);

    await summary.click();
    const list = frigateApp.page.getByTestId("status-issues-list");
    await expect(list.locator("li p")).toHaveText([
    ...
    await list
      .locator("li")
      .filter({ hasText: "CPU is slow (60 ms)" })
      .getByRole("link")
      .click();
