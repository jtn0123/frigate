...
test.describe("genai provider settings @medium @mobile", () => {
  test("a saved role the provider cannot confirm stays and is not dirty", async ({
    ...
    // The offered model proves capability data has reached the form.
    await frigateApp.page.locator(`#root_${ENTRY}_model`).click();
    await expect(
      frigateApp.page.getByRole("option", { name: "qwen3-asr" }),
    ).toBeVisible();
