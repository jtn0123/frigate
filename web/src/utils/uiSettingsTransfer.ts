...
async function readTransferable<T = unknown>(
  key: string,
  namespaced: boolean,
  username: string | undefined,
): Promise<T | undefined> {
  ...
  const value = await getData<T>(namespacedKey);
  ...
  await Promise.all(
    TRANSFER_KEYS.map(async (entry) => {
      ...
      if (entry.section === "streaming") {
        ...
        streaming = value as UiSettingsFile["sections"]["streaming"];
        return;
      }
  ...
  if (
    ...
    writes.push(
      (async () => {
        ...
        const existing =
          (await readTransferable<UiSettingsFile["sections"]["streaming"]>(
            streamingEntry.key,
