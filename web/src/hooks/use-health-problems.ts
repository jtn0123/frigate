...
export function useHealthProblems(
  ...
  const unhideAll = useCallback(async () => {
    ...
    await Promise.all([mutateHidden(), mutateMutedChecks()]);
  }, [mutateHidden, mutateMutedChecks]);

  const runAction = useCallback(
    (action: () => Promise<unknown>) => {
      void action().catch(() =>
        toast.error(t("health.notices.actionFailed", { ns: "views/system" })),
      );
    },
    [t],
  );
      ...
      return {
        ...
        ...(hiddenAt(notice) > 0
          ? {
              ...
              onUnhide: () => runAction(() => unhide(notice.id)),
            }
          : {
              onAcknowledge: notice.acknowledgeable
                ? () => runAction(() => acknowledge(notice.id))
                : undefined,
              onMute: () => runAction(() => mute(notice.id)),
            }),
      };
    },
    [acknowledge, mute, unhide, formatTime, t, runAction],
  );
    ...
    () =>
      sortHealthProblems([
        ...
        ...checks
          ...
          .map((check) => ({
            ...check,
            onMute: () => runAction(() => muteCheck(check.id)),
          })),
      ]),
    [notices, noticeRow, checks, mutedAt, muteCheck, runAction],
  );
          ...
          : [
              {
                ...
                row: {
                  ...
                  onUnhide: () => runAction(() => unmuteCheck(check.id)),
                },
              },
            ];
      }),
