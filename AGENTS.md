...
This repository is the fork `jtn0123/frigate`, not upstream. `FORK.md` is the ledger of every divergence and states the rules; `fork/README.md` describes the tooling. Commits, pushes and pull requests only ever go to `jtn0123/*`.

- **Worktree**: Start each piece of work with `make wt NAME=<name>`. It creates a worktree on branch `section/<name>` from `origin/next`, with `web/node_modules` and its own e2e port. The primary clone's `main` may be stale, so always compare against `origin/next` (after `git fetch origin`), never against a local `main`.
- **Branches**: Pull requests target `next` and are merged with merge commits, never `main`. `main` is the release branch and moves only through `make promote`, which the owner runs. `dev` mirrors upstream and is never committed to.
- **Ledger IDs**: Every change takes an unused ID, a category letter plus a number (like `C17`, `I24` or `UI64`). Check both `FORK.md` and `fork/GRADE-REPORT.md` before picking one: the report defines the open audit items, and doing one of those uses its ID. The commit subject starts with `<ID>: `, and the same change adds the ID's row as a new file `fork/ledger/<ID>.md` (one table row, see `fork/ledger/README.md`; never append to `FORK.md`, where every open pull request would conflict) or updates the row in place if `FORK.md`'s table already has it. Check `fork/ledger/` for taken IDs too. Release notes are grouped by these IDs, so never reuse one for something else.
- **Stay mergeable**: Add files under the `fork/` folders (`fork/`, `web/src/**/fork/`, `web/e2e/specs/fork/`, `web/scripts/fork/`) rather than editing upstream files. When an upstream file must change, keep the hunk small and self-contained, and never rename, move or reformat it. Fork-only UI behavior is gated in `web/src/fork/flags.ts`.
- **Gates**: Run `make check-fast` while working (changed-only) and `make check` before opening a pull request. `make check` runs what CI's "Fork - Checks" runs (see `fork/scripts/check.sh`): ESLint plus the e2e spec and fixture lint, `tsc` for the app and the e2e specs, the type ratchet (`fork/type-ratchet.json`), the i18n key check, ruff, gitleaks, vitest, the unit tests of `fork/scripts`, the e2e build with the bundle budget (`fork/bundle-budget.json`), the Playwright suite, and in Docker mypy, the API spec check and the backend unit tests. CI adds the SonarCloud quality gate (at least 80% coverage on new code; see `fork/SONAR-CI.md`), and `next` requires its "Checks passed" status.
- **Owner only**: Never push to `main`, run `make promote`, or create or read tokens and secrets. The owner's steps are in `fork/README.md` under "Owner setup".
