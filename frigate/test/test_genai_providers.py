...
class TestOllamaProvider(unittest.TestCase):
    ...
    def test_send_uses_chat_with_captions_before_each_image(self):
        ...
        with (
            patch("frigate.genai.plugins.ollama.generation_request") as accounting,
            patch("frigate.genai.plugins.ollama.record_generation_metrics") as metrics,
        ):
            result = client._send(
                "prompt",
                [b"a", b"b"],
        ...
        accounting.assert_called_once_with(2)
        accounting.return_value.__enter__.assert_called_once()
        accounting.return_value.__exit__.assert_called_once()
        metrics.assert_called_once()
        self.assertIs(metrics.call_args.args[0], client.genai_config)
        self.assertEqual(metrics.call_args.args[1], client.provider.chat.return_value)
