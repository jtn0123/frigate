...
class TestUniqueArchiveName(unittest.TestCase):
    ...
    Preserve display names for legacy exports, including files whose old
    generated filename differs from their label.
    """
    ...
    def test_preserves_display_name_for_legacy_file(self):
        ...
        self.assertEqual(
            ...
            "whatever the display name is.mp4",
        )

    def test_follows_a_renamed_file(self):
        export = self.build_export("/media/frigate/exports/Package thief_abc123.mp4")
        export.name = "Package thief"
        self.assertEqual(_unique_archive_name(export, set()), "Package thief.mp4")

    def test_entries_are_deduplicated(self):
        export = self.build_export("/media/frigate/exports/Package thief_abc123.mp4")
        export.name = "Package thief"
        used: set[str] = set()
        self.assertEqual(_unique_archive_name(export, used), "Package thief.mp4")
        self.assertEqual(_unique_archive_name(export, used), "Package thief_1.mp4")
