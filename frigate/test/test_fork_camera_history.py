...
class HistoryTestCase(unittest.TestCase):
    ...
    def history(self) -> CameraHistory:
        return CameraHistory(path=self.path)
...
class TestPersistence(HistoryTestCase):
    ...
    def test_an_unreadable_file_is_ignored(self):
        ...
        self.assertEqual(self.history().read("1h", now=NOW)["cameras"], {})
    ...
    def test_malformed_entries_are_dropped_on_load(self):
        ...
        data = self.history().read("1h", now=NOW)["cameras"]
