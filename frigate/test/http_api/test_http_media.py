...
class TestHttpMedia(BaseTestHttp):
    ...
    def test_vod_excludes_segments_ending_at_start_even_with_earlier_keyframes(self):
        """Coverage is half-open, so boundary-only rows cannot duplicate playback."""
        with AuthTestClient(self.app) as client:
            self._insert_recording(
                "previous_keyframe",
                REQUEST_START - 10,
            ...
            self._insert_recording("current", REQUEST_START, REQUEST_START + 10)
            response = client.get(
                f"/vod/front_door/start/{REQUEST_START}/end/{REQUEST_END}"
            )
            self._assert_vod_response(
                response,
                [("/media/recordings/current.mp4", None, 10000)],
            )
    ...
    def test_recordings_default_range_follows_the_request_clock(self):
        ...
        with AuthTestClient(self.app) as client:
            Recordings.insert(
                id="after_boot",
                path="/media/recordings/after_boot.mp4",
                camera="front_door",
                start_time=recorded_at - 20,
                end_time=recorded_at - 10,
                duration=10,
                motion=0,
                objects=0,
            ).execute()
