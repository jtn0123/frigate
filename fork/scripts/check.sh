...
py_gates='^(frigate|migrations|docker|fork/(audio_trial|monitoring|benchmarks))/|^[^/]+\.py$|^(Makefile|pyproject\.toml)$|^fork/(Dockerfile\.test|runtime-base\.env|requirements-dev\.lock|scripts/(targets|py-checks)\.sh|scripts/dev-lock-check\.py)$|^docs/static/frigate-api\.yaml$'
e2e_args=()
