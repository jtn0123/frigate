...
#### I9 — Faster test loop in CI and locally `[fork]`
- **Where:** `.github/workflows/fork-checks.yml` (single e2e job; thin test image rebuilt every run); `Makefile` (no changed-only e2e)
- **What's wrong:** Feedback time grows with every spec added.
- **Fix:** Shard Playwright across 2–3 runners; push the thin test image to GHCR keyed on its inputs; `make e2e-changed` using `--only-changed`.
- **Effort:** S
- **Grade lift:** B → B
