...
### PR-30 · sonar-fork — C12, D16, I14 — S — **in review (PR #29)**
- **Status (2026-09-11):** owner asked to "look at sonar and start whacking
  ...
  and D16 (e2e tags) on `section/sonar-fork`; I14 (scripts, CI, test image,
...
- **Follow-ups:** Sonar's rule descriptions could not be read from here, so
  the I14 fixes follow Sonar's usual compliant forms and count as cleared
  only once Sonar re-analyses main. Upstream
  `.github/workflows/pull_request.yml` has the same `npx` and pinning
  findings (lines 27, 48, 54, 69); left for an owner call.
